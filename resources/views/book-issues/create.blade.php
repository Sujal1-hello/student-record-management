<x-app-layout>
    ...
    <div class="form-container">
        ...
        <div class="page-intro">
            ...
            <a href="{{ route('book-issues.index') }}" class="clear-button">← Back to Book Issues</a>
        </div>
        ...
        <div class="form-card">
            <form method="POST" action="{{ route('book-issues.store') }}">
                        ...
                        <select name="book_id">
                            ...
                            @foreach ($books as $book)
                                <option value="{{ $book->id }}" {{ old('book_id') == $book->id ? 'selected' : '' }}>
                                    {{ $book->title }} ({{ $book->book_id }}) — {{ $book->available_copies }} available
                                </option>
                            @endforeach
                        </select>
                        @if ($books->isEmpty())
                            <small class="field-hint" style="color:#dc2626;">No books currently have available copies.</small>
                        @endif
                    </div>
                ...
                <div class="form-actions">
                    <a href="{{ route('book-issues.index') }}" class="clear-button">Cancel</a>
                    <button type="submit" class="save-button">Issue Book</button>
                </div>
